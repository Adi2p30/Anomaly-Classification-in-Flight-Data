...
Usage:
    python interactive_viewer.py [filename]
...
Without an evaluation on disk, it falls back to the first true-anomaly file
in labels.csv.
...
def reconstruct_predicted_window(values, fs, f0_hat, model):
    ...
    probas = [
        model.predict_proba(
            np.array([[feat.extract_window_features(values[s:e], fs, f0_hat)[c]
                       for c in feat.FEATURE_COLUMNS]])
        )[0, 1]
        for s, e in windows
    ]
    return reconstruct_window(starts, ends, probas, len(values))


def load_eval_per_file():
    path = config.MODEL_DIR / "eval_per_file.csv"
...
def choose_test_file(eval_df):
    ...
    for i, r in eval_df.iterrows():
        ...
        print(f"  [{i}] {r['filename']}  amp_ratio={r['amp_ratio']:.2f}  true={true_desc}  iou={r['iou']:.3f}")
    try:
...
def main():
    ...
    kind, filename = resolve_source(sys.argv[1] if len(sys.argv) > 1 else None, labels_df)
    ...
    if eval_row is not None:
        ...
        print(f"[test-set eval] predicted window={pred_desc} "
        ...
        true_desc = f"{true_start_s}s-{true_end_s}s" if true_start_s is not None else "none"
        print(f"estimated f0={f0_hat:.3f} Hz, predicted window="
              f"{pred_start_s}s-{pred_end_s}s (true={true_desc})")
    else:
        print("no trained model found at "
              f"{config.MODEL_PATH} -- run train_model.py first for the predicted overlay")
