...
def build_rows_for_file(row, config_mod):
    ...
    # pass 1: cheap per-window RMS, to build the per-file reference that
    # dc_offset_dev/rms_ratio need (amp_dropout/dc_jump are only detectable
    # relative to a file's own baseline scale, not in absolute terms)
    ...
    # pass 2: full feature extraction using file_ref
    rows = []
    for start, end in windows:
        window_values = values[start:end]
        window_feats = feat.extract_window_features(window_values, fs, f0_hat, file_ref)
